...
class BoundedContextTest extends TestCase
{
    ...
    public function testContextSetsPayloadAndVersionExplicitly(): void
    {
        $kernel = new DomainKernel('/app/src');
        // Caller has its own payload+version that should NOT propagate.
        $bc = new BoundedContext($kernel, ['caller-payload' => true], '1.0');

        $newPayload = ['command' => 'CreateOrder'];
        /** @var BoundedContext $child */
        $child = $bc->context(BoundedContext::class, $newPayload, '2.0');

        $this->assertInstanceOf(BoundedContextInterface::class, $child);

        $reflection = new \ReflectionClass($child);

        $payloadMethod = $reflection->getMethod('payload');
        $payloadMethod->setAccessible(true);
        $this->assertSame($newPayload, $payloadMethod->invoke($child));

        $versionMethod = $reflection->getMethod('version');
        $versionMethod->setAccessible(true);
        $this->assertSame('2.0', $versionMethod->invoke($child));
    }

    public function testContextThrowsLogicExceptionForNonBoundedContextClass(): void
    {
        $kernel = new DomainKernel('/app/src');
        $bc = new BoundedContext($kernel);

        $this->expectException(LogicException::class);
        $this->expectExceptionMessage('context() requires a');

        $bc->context(stdClass::class, ['payload' => 'x']);
    }

    public function testContextResolvesClassVersionAgainstNewVersion(): void
    {
        $proxyForV2 = new BoundedContext(new DomainKernel('/app/src'));

        $classVersion = $this->createMock(ClassVersionInterface::class);
        // Caller is on '1.0', but context() asks for 'v2' — ClassVersion must
        // be invoked with 'v2', not the caller's '1.0'.
        $classVersion->expects($this->once())
            ->method('__invoke')
            ->with(BoundedContext::class, 'v2')
            ->willReturn($proxyForV2);
        ...
        $result = $bc->context(BoundedContext::class, ['payload' => 'fresh'], 'v2');

        $this->assertSame($proxyForV2, $result);
    }
}
